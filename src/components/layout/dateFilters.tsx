...
import { ddMmYyyyToIsoString, firstDayOfYear, lastDayOfYear, lastFourWeeksMonday } from '@/utils/utils';
...
export default function DateFilters({ dateType, firstYear, options, defaultValue }: DateFiltersProps) {
  ...
  return (
    <>
      {dateType === 'calendarYear' || dateType === 'fullYear' ? (
        <Selector disabled={!firstYear} value={selectedDate} onHandleChange={handleChange} options={options} />
      ) : (
        <div className="flex flex-col gap-4 justify-center items-center mb-10">
          {dateType === 'allTime' && <h3>{`From ${firstYear}`}</h3>}
          {dateType === 'pastFourWeeks' && <h3>{`From ${lastFourWeeksMonday()}`}</h3>}
        </div>
      )}
    </>
  );
