...
export const athleteId = (state: ActivityState): number => {
  return state.cyclingRides?.[0]?.athleteId ?? 0;
};
...
const filteredByDateRides = (dateSection: DateSection) =>
  createSelector(
    [(state: ActivityState) => state.cyclingRides, (state: ActivityState) => state.filters[dateSection]],
...
const averageKmByDate = (dateSection: DateSection) =>
  createSelector([totalKmByDate(dateSection), totalActivities], (totalKmByDate, totalActivities): number => {
...
const activiesCountBetweenOneHundredAndTwoHundredKm = (state: ActivityState): number => {
  return state.cyclingRides.filter(activity => activity.distance > 100000 && activity.distance < 200000).length;
};

const activiesCountWithMoreThanTwoHundredKm = (state: ActivityState): number => {
  return state.cyclingRides.filter(activity => activity.distance > 200000).length;
};

const activeDaysCount = (state: ActivityState): number => {
  const uniqueDays = new Set(state.cyclingRides.map(activity => activity.start_date));
...
const sortedDistancesKm = (state: ActivityState): number[] => {
  return state.cyclingRides.map(activity => activity.distance / 1000).sort((a, b) => b - a);
};
...
export const rideAllTimesMetrics = createSelector(
  ...
  state => {
    return {
      ...
      highestCountOfConsecutiveActiveDays: highestCountOfConsecutiveActiveDays(state),
  ...
  createSelector(
    ...
    state => {
      return {
        ...
        totalElevationInMeters: totalElevationInMetersByDate(dateSection)(state),
